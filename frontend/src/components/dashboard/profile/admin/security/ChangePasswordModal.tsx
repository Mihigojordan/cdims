...
const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({ isOpen, onClose, adminId }) => {
  ...
  let fieldError = '';
  if (name === 'oldPassword') fieldError = validateOldPassword(value);
  if (name === 'newPassword') fieldError = validateNewPassword(value);
  if (name === 'confirmPassword') fieldError = validateConfirmPassword(value, name === 'confirmPassword' ? formData.newPassword : formData.newPassword);

  setErrors((prev) => {
    const newErrors = { ...prev };
    if (fieldError) {
      newErrors[name as keyof Errors] = fieldError;
    } else {
      delete newErrors[name as keyof Errors]; // ✅ clear error if valid
    }
    return newErrors;
  });
};
